...
/**
 * Domain interface to compute estimated electrification savings (Cost in € of kWh vs equivalent gasoline cost).
 */
interface GetElectrificationSavingsUseCase : FlowUseCase<GetElectrificationSavingsUseCase.Input, GetElectrificationSavingsUseCase.Output> {
...
class GetElectrificationSavingsUseCaseImpl @Inject constructor(
    private val expenseRepository: FuelExpenseRepository,
    private val logger: LoggerKit
) : GetElectrificationSavingsUseCase {
    ...
    override fun invoke(input: GetElectrificationSavingsUseCase.Input): Flow<GetElectrificationSavingsUseCase.Output> {
        ...
        return expenseRepository.getExpensesByVehicle(input.vehicleId).map { expenses ->
            ...
            if (electricExpenses.isEmpty()) {
                GetElectrificationSavingsUseCase.Output.Empty
            } else {
            ...
            .onStart { emit(GetElectrificationSavingsUseCase.Output.Progress) }
            .catch { e ->
                logger.e("GetElectrificationSavingsUseCase", "Error computing electrification savings", e)
                emit(GetElectrificationSavingsUseCase.Output.Failure)
            }
    }
}
